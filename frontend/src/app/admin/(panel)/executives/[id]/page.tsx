...
export default function EditExecutivePage() {
    ...
    const executiveId = Number(params.id);

    useEffect(() => {
        if (
            !Number.isInteger(executiveId) ||
            executiveId <= 0
        ) {
            setError(
                "El ID del directivo no es válido.",
            );
            setLoading(false);
            return;
        }
        ...
        async function loadExecutive() {
            try {
                ...
                const data =
                    await getExecutive(executiveId);
                ...
                setError(
                    err instanceof Error
                        ? err.message
                        : "No se pudo obtener el directivo.",
                );
            } finally {
                setLoading(false);
            }
    ...
    async function handleDeactivate() {
        ...
        const confirmed = window.confirm(
            `¿Deseas desactivar al directivo "${executive.full_name}"?\n\nEl registro no será eliminado de la base de datos.`,
        );
        ...
        try {
            ...
            setError(
                err instanceof Error
                    ? err.message
                    : "No se pudo desactivar el directivo.",
            );
        } finally {
    ...
    if (error || !executive) {
        return (
            <ErrorState
                message={
                    error ??
                    "No se encontró el directivo solicitado."
                }
            />
        );
    }
    ...
    return (
        <>
            ...
            <PageHeader
                ...
                description={`Modifica la información de ${executive.full_name}.`}
                action={
                    executive.is_active ? (
                        <button
                            type="button"
    ...
    return (
        <div className="flex min-h-[450px] flex-col items-center justify-center px-5 text-center">
            ...
            <p className="mt-4 text-sm font-semibold text-zinc-300">
                No se pudo cargar el directivo
            </p>
